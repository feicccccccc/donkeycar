...
# Custom model
# KerasIMU2, take in 2 IMU reading
# Keras_IMU_LSTM_Categorical,
...
class KerasLatent(KerasPilot):
    ...
    def run(self, img_arr):
        img_arr = img_arr.rteshape((1,) + img_arr.shape)
        outputs = self.model.predict(img_arr)
        steering = outputs[1]
        throttle = outputs[2]
        return steering[0][0], throttle[0][0]
...
class Keras_IMU_LSTM_Categorical(KerasPilot):
    ...
    def run(self, img_seq, imu_seq):
        ...
        #print("Raw prediction: ", angle_binned, "shape: ", angle_binned.shape)
...
class Keras_IMU_LSTM_Many2Many_Categorical(KerasPilot):
    """
    LSTM with 7 states
    """
    def __init__(self, image_w =224, image_h=224, image_d=3, seq_length=7, future_step = 5, num_imu_input=12, *args, **kwargs):
        super(Keras_IMU_LSTM_Many2Many_Categorical, self).__init__(*args, **kwargs)
        self.image_shape = (image_h, image_w, image_d)
        self.seq_length = seq_length
        self.future_step = future_step
        self.num_imu_input = num_imu_input
        self.model = lstm_imu_many2many_categorical(img_in = self.image_shape,
...
def lstm_imu_many2many_categorical(img_in=(224, 224, 3), imu_in=12, seq_length=7, future_step = 5):
    ...
    z = CuDNNGRU(64, return_sequences=True, activation='tanh')(z)
...
class Keras_IMU_LSTM_Many2Many_IMUPRED(KerasPilot):
    """
    LSTM with 7 states
    """
    ...
    def compile(self):
        self.model.compile(optimizer=self.optimizer,
                           loss={'angle_out': 'categorical_crossentropy',
                                 'throttle_out': 'categorical_crossentropy',
                                 'imu_out': 'mean_absolute_error' },
                           loss_weights={'angle_out': 1, 'throttle_out': 1, 'imu_out': 10})
    ...
    def run(self, img_seq, imu_seq):
        ...
        img_seq = img_seq.reshape(1, img_seq.shape[0], img_seq.shape[1], img_seq.shape[2], img_seq.shape[3])
        imu_seq = imu_seq.reshape(1, imu_seq.shape[0],imu_seq.shape[1])
        angle_binned, throttle_binned, imu_pred = self.model.predict([img_seq,imu_seq])
